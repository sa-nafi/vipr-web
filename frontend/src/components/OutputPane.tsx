...
export const OutputPane: FC<OutputPaneProps> = ({
  ...
  const renderStatusBadge = () => {
    switch (status) {
      case 'running':
        return (
          <div className={`badge badge-sm gap-1.5 py-2 font-mono text-[11px] ${
            isLight
      ...
      case 'waking_up':
        return (
          <div className={`badge badge-sm gap-1.5 py-2 font-mono text-[11px] animate-pulse ${
            isLight
  ...
  return (
    <div className={`flex flex-col h-full w-full overflow-hidden relative ${isLight ? 'bg-slate-50' : 'bg-base-100'}`}>
      {/* Terminal Header */}
      <div className={`flex-none h-10 flex items-center justify-between px-3 border-b select-none ${
        isLight
        ...
        <div className="flex items-center gap-3">
          ...
          <div className={`flex items-center gap-1.5 font-mono text-xs font-semibold ${
            isLight ? 'text-slate-800' : 'text-base-content/80'
          }`}>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={handleCopy}
            disabled={!fullOutput}
            className={`btn btn-ghost btn-xs gap-1 font-mono text-[11px] border disabled:opacity-30 ${
              isLight
          ...
          <button
            ...
            className={`btn btn-ghost btn-xs btn-square border disabled:opacity-30 ${
              isLight
                ? 'border-slate-300 hover:border-slate-400 text-slate-700 bg-white/60'
                : 'border-base-content/10 hover:border-base-content/25'
            }`}
      ...
      <div className={`flex-1 w-full p-4 overflow-y-auto font-mono text-xs md:text-sm ${
        isLight
          ...
          <div className={`flex flex-col items-center justify-center h-full gap-4 py-8 select-none ${
            ...
            <div className="text-center max-w-xs space-y-1">
              ...
              <p className={`text-xs font-sans ${isLight ? 'text-slate-700' : 'text-neutral-300'}`}>
                ...
                <kbd className={`kbd kbd-xs font-mono ${
                  isLight
            ...
            <div className={`grid grid-cols-2 gap-2 text-[11px] font-sans mt-2 ${
              ...
              <div className={`p-2 rounded border flex items-center gap-1.5 ${
                isLight ? 'bg-white border-slate-200 shadow-sm' : 'bg-neutral-900/60 border-neutral-800/80'
              }`}>
            ...
            {result.stdout && (
              <div className="space-y-1">
                <div className={`text-[10px] uppercase tracking-wider font-semibold ${
                  isLight ? 'text-blue-700' : 'text-blue-400/80'
                }`}>
                  Standard Output
                </div>
                <pre className={`whitespace-pre-wrap font-mono leading-relaxed p-3 rounded-lg border ${
                  isLight
            ...
            {result.stderr && (
              <div className="space-y-1">
                <div className={`text-[10px] uppercase tracking-wider font-semibold flex items-center gap-1 ${
                  isLight ? 'text-rose-700' : 'text-rose-400/80'
                }`}>
                  <AlertCircle className="w-3 h-3" />
      ...
      {/* Terminal Footer Bar */}
      <div className={`flex-none h-7 flex items-center justify-between px-3 border-t text-[11px] font-mono select-none ${
        isLight
          ? 'bg-slate-200/70 border-slate-300/80 text-slate-700'
